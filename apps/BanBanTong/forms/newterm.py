...
class NewTermForm(forms.ModelForm):
    ...
    def clean_school_year(self):
        ...
        q = models.NewTerm.objects.filter(school_year=data)  # .exclude(deleted=True)
    ...
    def clean_start_date(self):
        ...
        if not self.instance._state.adding:
            return data
        q = models.NewTerm.objects.filter(start_date__lte=data,
                                          end_date__gte=data)
    ...
    def clean(self):
        cleaned_data = super(NewTermForm, self).clean()
            ...
            if start_date >= end_date:
                raise forms.ValidationError(u'学期开始时间(%(s)s)必须早于结束时间(%(e)s)！',
                                            params={'s': str(start_date),
                                                    'e': str(end_date)})
            q = models.NewTerm.objects.filter(start_date__gte=start_date,
        ...
        if school_year and term_type:
            q = models.NewTerm.objects.filter(school_year=school_year,
                                              term_type=term_type)
            #q = q.exclude(deleted=True)
            if q.exists():
        ...
        if models.NewTerm.objects.filter(deleted=False).exists():
            raise forms.ValidationError(u'存在未结转的学年学期，请先结转!')

        return cleaned_data

    class Meta:
        model = models.NewTerm
        exclude = ['uuid', 'country']


NewTermFormSet = formset_factory(NewTermForm)


class NewTermUploadForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        super(NewTermUploadForm, self).__init__(*args, **kwargs)
        self.fields['excel'].error_messages = {
            'required': '请上传学年学期的xls文件！'
        }

    def clean(self):
        cleaned_data = super(NewTermUploadForm, self).clean()
        ...
        if excel:
            ...
            if ext not in self._valid_extensions:
                ...
                if len(sheet.row_values(0)) < len(self.head):
                    ...
                    logger.debug('Term import excel head not match!')
    ...
    def save(self, commit=True):
        ...
        for l in parse_xls(self.sheet):
            ...
            if form.is_valid():
                ...
                logger.debug('Term excel: invalid: l=%s, err=%s', l, form.errors)
        return ret
