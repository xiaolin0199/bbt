...
def query(cond, excludes):
    ...
    total = q.count()
    #q = q.order_by('country_name', 'school_name', 'grade_name', 'class_name')
...
@decorator.authorized_privilege('resource_from_statistic')
def by_town(request):
    cache_key = 'resource-from-by-town'
    fields = ('town_name', 'resource_from',)
...
def by_school(request):
    cache_key = 'resource-from-by-school'
...
def by_class(request):
    cache_key = 'resource-from-by-class'
    fields = ('town_name', 'school_name', 'grade_name', 'class_name', 'resource_from',)
    excludes = {'resource_from': ''}
    ret = _result(request, cache_key, fields, excludes)
    return ret
...
def by_lesson(request):
    cache_key = 'resource-from-by-lesson'
    fields = ('town_name', 'school_name', 'lesson_name', 'resource_from',)
    excludes = {'resource_from': ''}
    ret = _result(request, cache_key, fields, excludes)
    return ret
...
def by_teacher(request):
    cache_key = 'resource-from-by-teacher'
    fields = ('town_name', 'school_name', 'teacher_name', 'resource_from',)
    excludes = {'resource_from': ''}
    ret = _result(request, cache_key, fields, excludes)
    return ret
...
def _export(cache_key, query_fields,
    ...
    filename = u'资源来源使用统计_按%s导出.xls' % d[node_type]
